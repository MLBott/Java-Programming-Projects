...
public class MyIntSET {
	...
	private int sizeOdd(Node x) {
		...
		return sizeOdd(x.left) + sizeOdd(x.right) + j; 
	}
	...
	private int sizeAtDepth(int k, Node x, int depth){
    	...
    	return sizeAtDepth(k, x.right, depth + 1) + sizeAtDepth(k, x.left, depth + 1) + j;
    }
	...
	public int sizeAboveDepth(int k) {
		// TODO
		return sizeAboveDepth(k, root, 0);
	}
	...
	private int sizeAboveDepth(int k, Node x, int depth){
    	...
    	return sizeAboveDepth(k, x.left, depth + 1) + sizeAboveDepth(k, x.right, depth + 1) + j;
	}
	...
	public int sizeBelowDepth(int k) {
		// TODO
		return sizeBelowDepth(k, root, 0);
	}
	...
	private int sizeBelowDepth(int k, Node x, int depth){
    	...
    	return sizeBelowDepth(k, x.left, depth + 1) + sizeBelowDepth(k, x.right, depth + 1) + j;
    }
	...
	public boolean isPerfectlyBalancedS() {
		// TODO
		return isPerfectlyBalancedS(root) > -1;
	}

	private int isPerfectlyBalancedS(Node x){
    	if (x == null) return 0;
    	int rightSide = isPerfectlyBalancedS(x.right);
    	int leftSide = isPerfectlyBalancedS(x.left);
    	if ((rightSide == leftSide) && (leftSide != -1)) return leftSide + 1;
    	return -1;
    }
	...
	public boolean isPerfectlyBalancedH() {
		// TODO
		return isPerfectlyBalancedH(root) > -2;
	}

	private int isPerfectlyBalancedH(Node x){
    	if (x == null) return 0;
    	int leftSide = isPerfectlyBalancedH(x.left);
    	int rightSide = isPerfectlyBalancedH(x.right);
    	if ((rightSide == leftSide) && (leftSide != -2)) return leftSide + 1; 
    	return -2;
    }
	...
	public boolean isOddBalanced() {
		// TODO
		return isOddBalanced(root) > -1;
	}
	
	private int isOddBalanced(Node x){
    	if (x == null) return 0;
    	int rightSide = isOddBalanced(x.right);
    	int leftSide = isOddBalanced(x.left);
    	if ((rightSide == leftSide) && (leftSide != -1)) return leftSide + (x.key % 2 == 1 ? 1 : 0);
    	return -1;
    }
	...
	private int isSemiBalanced(Node x){
    	...
    	else if ((x.right == null && x.left != null) || (x.left == null && x.right != null))  {
    		return 0;
    	} else {
    		int rightSize = size(x.right);
    		int leftSize = size(x.left);
    		if (leftSize > rightSize) {
    			if (leftSize <= 3 *rightSize ) {
    				return 1 + isSemiBalanced(x.left)+ isSemiBalanced(x.right);
    			} else {
    				return 0;
    			}
    		} else {
    			if (rightSize <= 3 * leftSize) {
    				return 1 + isSemiBalanced(x.left) + isSemiBalanced(x.right); 
    			} else {
    				return 0;
    			}
	...
	public void removeBelowDepth(int k) {
		// TODO
		removeBelowDepth(root, 0, k);
	}
	
	private void removeBelowDepth(Node x, int currentDepth, int k) {
		if (x == null) return;
		if (currentDepth == k) {
			x.left = null;
			x.right = null;
		...
		removeBelowDepth(x.right, currentDepth + 1, k);
		removeBelowDepth(x.left, currentDepth + 1, k);
	}
