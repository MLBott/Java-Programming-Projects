...
public class MyFB {
	...
	public void addPerson (Person p) {
		...
		if (!(map.containsKey(p)) && !p.name.equals(null)) {
	...
	public void addFriendship (Person p1, Person p2) {
		...
		if (map.containsKey(p1) && map.containsKey(p2)) {
			if (!(p1.name.equals(null)) && !(p2.name.equals(null))) {
				HashSet<Person> hashP1 = map.get(p1);
				if (!(hashP1.contains(p2))) {
					hashP1.add(p2);
				}
				HashSet<Person> hashP2 = map.get(p2);
			
				if (!(hashP2.contains(p1))) {
					hashP2.add(p1);
				}
			}
		}
